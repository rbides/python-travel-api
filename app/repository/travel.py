...
class SQLFilters(BaseModel):
    name: Annotated[
        str | None, PlainSerializer(lambda x: TravelEntity.name.contains(x))
    ] = None
    min_price: Annotated[
        Decimal | None, PlainSerializer(lambda x: TravelEntity.price >= x)
    ] = None
    max_price: Annotated[
        Decimal | None, PlainSerializer(lambda x: TravelEntity.price <= x)
    ] = None
    min_departure: Annotated[
        datetime | None, PlainSerializer(lambda x: TravelEntity.departure >= x)
    ] = None
    max_departure: Annotated[
        datetime | None, PlainSerializer(lambda x: TravelEntity.departure <= x)
    ] = None
...
def get(filters: TravelFilters) -> list[Travel]:
    sql_filters = SQLFilters(
        **filters.model_dump(exclude_none=True, exclude="order_by")
    )
