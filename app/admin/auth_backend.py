...
class AdminAuth(AuthenticationBackend):
    ...
    async def authenticate(self, request: Request) -> bool:
        ...
        try:
            # TODO: Would be better to check for role instead,
            # but it's not being passed to the token currently.
            auth.verify_permissions(
                security_scopes=SecurityScopes(
                    [
                        Permissions.TRAVEL_WRITE.value,
                        Permissions.TRAVEL_DELETE.value,
                    ]
                ),
...
)  # Is it ok to use the same key we used for Oauth?
