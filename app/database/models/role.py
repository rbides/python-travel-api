from sqlalchemy import UUID, Column, Enum, ForeignKey, String, DateTime, Table
...
class RoleEntity(Base):
    ...
    permissions = relationship("PermissionEntity", secondary=role_permissions_association)

class PermissionEntity(Base):
    __tablename__ = 'permissions'
    id = Column(UUID, primary_key=True)
    permission = Column(Enum(enums.Permissions, values_callable=lambda e: [p.value for p in e]), nullable=False, unique=True) # lambda to store enum values instead of names in db
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), server_onupdate=func.now())
...
# Not using bridge tabels here since User will only have 1 role (1 x N relationship)
